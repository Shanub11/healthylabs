...
class MedicalVectorService:
    ...
    def __init__(self, uri: str, user: str, password: str):
        ...
        visual_model_name = os.getenv("VISUAL_EMBEDDING_MODEL", "clip-ViT-B-32")
    ...
    async def _batch_embed_text(self, texts: List[str]) -> List[List[float]]:
        ...
        embeddings = self.text_model.encode(texts, batch_size=self.MAX_BATCH_SIZE)
    ...
    async def _batch_embed_visual(self, image_paths: List[str]) -> List[List[float]]:
        ...
        embeddings = self.visual_model.encode(images, batch_size=self.MAX_BATCH_SIZE)
        return embeddings.tolist()
