...
# main.py — add at the very top, before any other imports
import sys
import asyncio
...
async def startup_event():
    run_schema_migrations()
    logger.info("Database schema migrations ensured.")
...
async def diagnostic_chat(request: DiagnosticChatRequest):
    ...
    generation_prompt = f"""
    You are a highly capable clinical AI diagnostic assistant for HealthyLabs.
    Assess the user's query based STRICTLY on the provided Knowledge Context and the User's Personal Profile. Do not guess outside this knowledge.
