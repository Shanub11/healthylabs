...
def assert_ok(response, step):
    if response.status_code >= 400:
        raise RuntimeError(f"{step} failed with HTTP {response.status_code}. See response above.")
...
def test_ingestion():
    ...
    assert_ok(response, "Document ingestion")
...
def wait_for_vectors(timeout_seconds=90):
    ...
    while time.time() < deadline:
        ...
        print_response(response)
        assert_ok(response, "Vector status")
        data = response.json()
...
def test_chat():
    ...
    response = requests.post(url, json=payload)
    print_response(response)
    assert_ok(response, "Diagnostic chat")
