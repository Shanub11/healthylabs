...
class MedicalSafetyEngine:
    ...
    def __init__(self, settings):
        ...
        if _HAS_PRESIDIO:
            try:
                presidio_model = os.getenv("PRESIDIO_SPACY_MODEL", "en_core_sci_md")
                provider = NlpEngineProvider(
                    nlp_configuration={
                        "nlp_engine_name": "spacy",
                        "models": [{"lang_code": "en", "model_name": presidio_model}],
                    }
                )
                ...
                logger.warning(
                    "Presidio initialization failed; falling back to regex-only PHI detection: %s",
                    exc,
                )
